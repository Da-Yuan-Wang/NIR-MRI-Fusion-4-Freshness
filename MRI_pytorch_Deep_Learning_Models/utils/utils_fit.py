...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        print('Total Loss: %.3f || Val Loss: %.3f || Train Accuracy: %.3f || Val Accuracy: %.3f' % (total_loss / epoch_step, val_loss / epoch_step_val, total_accuracy / epoch_step, val_accuracy / epoch_step_val))
        
        # Use new saving mechanism, only keep top 3 models with highest validation accuracy
        loss_history.save_best_models(model, epoch, total_loss / epoch_step, val_loss / epoch_step_val, val_accuracy / epoch_step_val)