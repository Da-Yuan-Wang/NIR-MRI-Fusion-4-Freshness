...
def draw_plot_func(values, name_classes, plot_title, x_label, output_path, tick_font_size = 12, plt_show = True):
    fig     = plt.gcf() 
    axes    = plt.gca()
    plt.barh(range(len(values)), values, color='forestgreen')
...
def show_results(miou_out_path, hist, Recall, Precision, name_classes, tick_font_size = 12):
    ...
    # Draw confusion matrix and F1-score chart
    plt.figure(figsize=(12, 6))
    
    # Draw confusion matrix
    plt.subplot(121)
    im = plt.imshow(hist, interpolation='nearest', cmap=plt.cm.Greens)
    plt.title("GhostNet (MRI)", fontsize=18)
    cbar = plt.colorbar(im)
    cbar.ax.tick_params(labelsize=16)
    tick_marks = np.arange(len(name_classes))
    # # Modify label display to two lines
    # name_classes_multiline = []
    # for name in name_classes:
    #     # Convert camel case naming to format with line breaks
    #     if 'Fresh' in name:
    #         name_classes_multiline.append(name)
    #     else:
    #         # Simple processing, replace '-' with line break
    #         name_classes_multiline.append(name.replace('-', '\n') if '-' in name else name)
    
    # Directly specify multi-line label names
    name_classes_multiline = ['Class-S', 'Class-A', 'Class-B', 'Class-C']

    plt.xticks(tick_marks, name_classes_multiline, rotation=45, fontsize=16)
    plt.yticks(tick_marks, name_classes_multiline, fontsize=16)
    ...
    for i in range(len(name_classes)):
        for j in range(len(name_classes)):
            plt.text(j, i, format(hist[i, j], 'd'),
                    horizontalalignment="center",
                    fontsize=16,
                    color="white" if hist[i, j] > thresh else "black")
    
    # Calculate F1-score and draw bar chart
    plt.subplot(122)
    ...
    x = range(len(name_classes))
    plt.bar(x, f1_scores, color='forestgreen')
    # Use multi-line labels
    plt.xticks(x, name_classes_multiline, fontsize=16)
    ...
    plt.tight_layout()
    cm_path = os.path.join(miou_out_path, "Confusion_Matrix_and_F1_Score.png")
    plt.savefig(cm_path)

    plt.show()
    #plt.close()
    
    print("Save Confusion Matrix and F1-Score out to " + cm_path)
