...
# Read training data
train_data = pd.read_csv('data/train_data_Raw_GLCM.csv')
...
# Read test data
test_data = pd.read_csv('data/test_data_Raw_GLCM.csv')
...
feature_names = list(train_data.columns[1:-1]) # Feature names list
...
base_classifiers = [
    ...
    ('lr', LogisticRegression(solver='liblinear',         # Solver suitable for small datasets
                              ...
                              random_state=42,
                              max_iter=1000)),               # Increase maximum iterations to ensure convergence),  # Use logistic regression as base classifier
    ('svc', SVCWrapper(model_path='NIR-MRI(GLCM)_sklearan_Machine_Learning_Models\svc_model-NIR-MRI-GLCM-used.pkl')),  # Use saved SVC model
    #('mlp', MLPWrapper(model_path='NIR-MRI(GLCM)_sklearan_Machine_Learning_Models\mlpc_model-NIR-MRI-GLCM.pkl')),  # Use saved MLP model
    ("cnn", CNNWrapper(model_path=r'NIR-MRI(GLCM)_sklearan_Machine_Learning_Models\best_1D_CNN_NIR-MRI-GLCM_model_optimized.pth'))     # Use custom CNNWrapper as base classifier
]

# Train Stacking classifier (meta-classifier: logistic regression)
# Use classifier that supports probability prediction as meta-classifier
stacking_clf = StackingClassifier(estimators=base_classifiers, final_estimator=LogisticRegression(), cv=5)
#stacking_clf = StackingClassifier(estimators=base_classifiers, final_estimator=SVC(kernel='rbf'), cv=5)
...
plt.title("Stacking (NIR-MRI(GLCM))", fontsize=18)
...
try:
    ...
    results_df.to_csv('NIR-MRI(GLCM)_sklearan_Machine_Learning_Models/stacking_results/predictions_nir_mri_glcm.csv', index=False)
    print(f"\nPrediction results saved to: NIR-MRI(GLCM)_sklearan_Machine_Learning_Models/stacking_results/predictions_nir_mri_glcm.csv")
