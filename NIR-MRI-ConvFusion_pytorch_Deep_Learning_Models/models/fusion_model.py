...
class NIRFeatureExtractor(nn.Module):
    ...
    Based on the model structure in NIR-4-1D CNN-train-optimized version.py
    """
    def __init__(self, input_size):
        ...
        self.flattened_size = (input_size // 4) * 32  # After two pooling operations, size becomes 1/4 of original
        
        # Fully connected layer for feature extraction
        self.fc1 = nn.Linear(self.flattened_size, 128)  # Change output dimension from 64 to 128
...
class FusionModel(nn.Module):
    ...
    Note: NIR feature dimensions will be automatically adjusted based on input size
    """
    def __init__(self, nir_input_size, num_classes=4, fusion_type='concat', pretrained_mri_path=None):
        super(FusionModel, self).__init__()
        
        self.fusion_type = fusion_type
        self.nir_input_size = nir_input_size  # Save NIR input size
        self.num_classes = num_classes  # Save number of classes
        ...
        if fusion_type == 'concat':
            ...
            # NIR feature dimension is automatically calculated and related to input spectral length
            self.classifier = nn.Sequential(
                nn.Linear(nir_feature_dim + 128, 128),  # Input layer includes both NIR and MRI features
                nn.BatchNorm1d(128),
                nn.ReLU(),
                nn.Dropout(0.7),
            ...
            # Addition fusion: first project NIR features to the same dimension as MRI features
            # Project NIR features from nir_feature_dim to 128 dimensions through linear transformation
            self.nir_projection = nn.Linear(nir_feature_dim, 128)
            self.classifier = nn.Sequential(
                nn.Linear(128, 128),  # Input is the fused 128-dimensional features
                nn.BatchNorm1d(128),
                nn.ReLU(),
                nn.Dropout(0.7),
                nn.Linear(128, num_classes)  # Output layer
